...
char *fp2char_paded(data d, struct FORMAT *format) {
   ...
   if(format->exp == 0) {	//rational format applies
      ...
      if(n<=width) {
         ...
         /* leading minus */
         if(format_buffer[n]=='-') format_buffer[n++]=C_OVERBAR;
         /* up to decimal place */
         for(;format_buffer[n]!='.' && n<width;n++);
         /* count out d_places digits */
         for(m=0;m<format->right_ratn && n<width ;) {
	    if(format_buffer[n] != '.') m++;
            n++; 
         }
      }
      else {	// it didn't fit
         for(n=0;n<format->digits;n++) format_buffer[n]='*';
      }

      /* add trailing spaces */
      for(;n<width;n++) format_buffer[n]=' '; 

      /* hack to overcome -0 problem */
      n--;
      for(;format_buffer[n]=='0' || format_buffer[n]==' ' ||
           format_buffer[n]=='.';n--);
      if(format_buffer[n]==C_OVERBAR) format_buffer[n]=' ';
   }
      ...
      /* leading sign */
      for(n=0;format_buffer[n]==' ';n++);
      if(format_buffer[n]=='-' && value!=0 ) format_buffer[n++]=C_OVERBAR;
      else format_buffer[n++]=' ';
      ...
      /* add trailing spaces */
      for(;n<format->digits;n++) format_buffer[n]=' '; 
      format_buffer[n]='\0';
